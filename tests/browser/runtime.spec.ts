...
test("EAA chat streams once, survives reload, and exposes runtime controls", async ({ page, request }) => {
  ...
  expect((await request.post("/api/permissions", { data: { auto_allow: true } })).ok()).toBeTruthy();
  await page.goto("/");
...
test("pending approval survives reconnect and plan controls block launch", async ({ page, request }) => {
  ...
  const autoAllow = page.getByRole("switch", { name: "Auto-allow tool requests" });
  ...
  await expect(page.getByRole("button", { name: "Allow for session", exact: true })).toBeVisible();
  ...
  await page.getByRole("button", { name: "Settings", exact: true }).click();
  await page.getByRole("switch", { name: "Auto-allow tool requests" }).check();
  await page.getByRole("button", { name: "Chat", exact: true }).click();
