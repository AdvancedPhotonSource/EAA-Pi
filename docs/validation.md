...
| Pi distribution | pi-experiment-ops 0.2.0 | Independent CLI/resources/Python installer; versioned tarball dependency |
...
Backend restructuring validation for `pi-experiment-ops` 0.2.0 on 2026-09-17:

| Check | Command | Result |
|---|---|---|
| Server/frontend build | `npm run build` | Passed |
| Unit and integration | `npm test` | Passed: 32 checks, including a single installed Pi package, native provider configuration, legacy migration, TUI/WebUI session handoff, and process/workflow recovery |
| Standalone tarball | `npm run test:package` | Passed: one Pi installation, idempotent installer, chat, eight extensions, and real graph execution |

Experiment-ops' own tests cover native SDK export identity, side-effect-free workspace resolution, standalone TUI/provider behavior, and actual graph execution. Its package suite tests installation and SDK access outside the checkout. Fixture endpoints are local; live Argo authentication and inference were not exercised.

Browser and container acceptance previously passed against 0.1.1; they were not rerun for this backend restructuring.
