...
if [[ -d src ]]; then npm ci --no-audit --no-fund; elif ! node --input-type=module -e 'import "pi-experiment-ops/sdk"; import "pi-experiment-ops"' >/dev/null 2>&1; then npm install --omit=dev --legacy-peer-deps --no-audit --no-fund; fi
